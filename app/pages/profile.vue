<template>
  <div class="app-theme-transition container mx-auto px-4 py-8">
    <h1 class="ml-4 mb-8 text-4xl font-bold text-teal-700">{{ currentUser?.username }}</h1>
    <div v-if="isBusy" class="flex justify-center">
      <VueSpinner size="20" color="teal" />
    </div>
    <div v-else class="profile-card mb-12 rounded-sm p-8">
      <!-- Readonly, show the member since and e-mail address -->
      <div class="flex">
        <p class="font-bold">E-mail:</p>
        <p class="ml-4">{{ currentUser?.email }}</p>
      </div>
      <div class="flex">
        <p class="font-bold">Member since:</p>
        <p class="ml-4">{{ userCreatedTimeStamp }}</p>
      </div>
    </div>
    <div class="profile-card mb-12 rounded-sm p-8">
      <div v-if="isBusy" class="flex justify-center">
        <VueSpinner size="20" color="teal" />
      </div>
      <div v-else class="mb-4 flex items-center gap-x-2">
        ...
        <label for="username" class="block w-36 font-bold">Username:</label>
        <input maxlength="200" :disabled="isBusy" type="text" id="username"
          class="profile-input w-full rounded border p-2" v-model="username"
          v-click-outside="handleUsernameClickOutside" />
      </div>
    ...
    <div class="profile-card mb-12 rounded-sm p-8">
      ...
      <form v-else @submit.prevent="handleSubmitUpdatePassword">
        <div class="mb-4 flex items-center gap-x-2">
          <label for="password1" class="block w-36 font-bold">New password:</label>
          <input type="password" id="password1" class="profile-input w-full rounded border p-2" v-model="password1" />
        </div>
        <div class="mb-4 flex items-center gap-x-2">
          <label for="password2" class="block w-36 font-bold">Confirm password:</label>
          <input type="password" id="password2" class="profile-input w-full rounded border p-2" v-model="password2" />
        </div>
        <div class="mb-4">
          <p class="text-sm text-teal-700">*Note: Changing your password will end your session, and will require you to
            sign in again.</p>
        </div>
        <div v-if="passwordError" class="text-red-500">
          <p class="text-sm">{{ passwordError }}</p>
        </div>
        <SpinnerButton :isBusy="isBusy" :disabled="isInvalidPassword" type="submit" title="Update password"
